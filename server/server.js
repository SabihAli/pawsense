...
console.log("[SERVER] URL:", supabaseUrl ? `${supabaseUrl.substring(0, 20)}...` : "(missing)");
console.log("[SERVER] Key length:", supabaseServiceKey ? supabaseServiceKey.length : 0);

if (!supabaseUrl || !supabaseServiceKey || supabaseServiceKey === "your_service_role_key_here") {
  console.error("[SERVER] ❌ SUPABASE_URL or SUPABASE_SERVICE_KEY is missing or not configured in .env!");
  console.error("[SERVER] Get your service_role key from: https://supabase.com/dashboard/project/qsoafxxpejxlzxeuckgn/settings/api");
}

const supabase = createClient(
  supabaseUrl || "https://qsoafxxpejxlzxeuckgn.supabase.co",
  supabaseServiceKey || ""
);

console.log("[SERVER] Supabase project URL:", supabaseUrl || "(not set)");
